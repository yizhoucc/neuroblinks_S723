...
int param_laserrampoff = 0; //ALvaro 10/19/18 time to start the ramp off
...
StimulusRepeating laser(param_campretime + param_laserdelay, param_laserdur, laserOn, laserOff, 0, param_laserperiod, param_lasernumpulses, param_laserrampoff); //ALvaro 10/19/18
  ...
  while (Serial.available() > 2) {
    ...
    switch (header) {
      ...
      case 21:
        ...
        break;
      case 22:
        param_laserrampoff = value; //ALvaro 10/19/18
        break;
      case 23:
...
void laserrampoff(int dummy) { // Function signature requires int but we don't need it so call it "dummy"
    double counter;
    if (param_rampoffdur==1)
    for (counter = 4095; counter > 0; counter = counter-16)
        {
         DACWrite(powerToDACUnits(counter));
         }
    else
      if (param_laserpower==2)
      for (counter = 4095; counter > 0; counter = counter-8)
          {
           DACWrite(powerToDACUnits(counter));
          }};
